...
function jtpl_function_xml_jlocale($tpl, $locale)
{
    if (func_num_args() == 4 && is_array(func_get_arg(2))) {
        ...
        echo htmlspecialchars(Locale::get($locale, $param2, $param3));
    } elseif (func_num_args() == 3 && is_array(func_get_arg(2))) {
        $param = func_get_arg(2);
        echo htmlspecialchars(Locale::get($locale, $param));
    } elseif (func_num_args() > 2) {
        $params = func_get_args();
        unset($params[0], $params[1]);

        echo htmlspecialchars(Locale::get($locale, $params));
    } else {
        echo htmlspecialchars(Locale::get($locale));
    }
}
