...
//]]>
    ...
    {
        if ($ctrl->type == 'hidden' || $ctrl->type == 'group' || $ctrl->type == 'button') {
            return;
        }
    ...
    protected function jsInput($ctrl)
    {
        ...
        } elseif ($ctrl->datatype instanceof jDatatypeLocaleTime) {
            $dt = 'Time';
            $isLocale = true;
    ...
    protected function _outputDateControlSeconds($ctrl, $attr, $value, $configParameter = 'controls.datetime.input')
    {
        ...
        } elseif (jApp::config()->forms[$configParameter] == 'textboxes') {
            $attr['value'] = $value;
    ...
    protected function outputChoice($ctrl, &$attr)
    {
        ...
        foreach ($ctrl->items as $itemName => $listctrl) {
            ...
            foreach ($listctrl as $ref => $c) {
                if (!$this->_form->isActivated($ref) || $c->type == 'hidden') {
                    continue;
                }
